    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
                <InputField label="Prénom" name="firstName" value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} required />
                <InputField label="Nom" name="lastName" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })} required />
                <InputField label="Téléphone" name="phone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} required />
                <InputField label="Email" value={form.email} disabled />

                {isCompany && (
                    <>
                        <InputField label="Structure" name="companyName" value={form.companyName} onChange={(e) => setForm({ ...form, companyName: e.target.value })} required />
                        <InputField label="Service" name="service" value={form.service} onChange={(e) => setForm({ ...form, service: e.target.value })} />
                        <InputField label="Fonction" name="jobTitle" value={form.jobTitle} onChange={(e) => setForm({ ...form, jobTitle: e.target.value })} required className="md:col-span-2" />
                    </>
                )}

                {!isCompany && (
                    <>
                        <InputField label="Date de naissance" name="birthDate" value={form.birthDate} onChange={(e) => setForm({ ...form, birthDate: e.target.value })} required />
                        <InputField label="Lieu de naissance" name="birthPlace" value={form.birthPlace} onChange={(e) => setForm({ ...form, birthPlace: e.target.value })} required />
                        <InputField label="Pays de naissance" name="birthCountry" value={form.birthCountry} onChange={(e) => setForm({ ...form, birthCountry: e.target.value })} required />
                        <InputField label="Nationalité" name="nationality" value={form.nationality} onChange={(e) => setForm({ ...form, nationality: e.target.value })} required />
                        <InputField label="N° Siret" name="siretNumber" value={form.siretNumber} onChange={(e) => setForm({ ...form, siretNumber: e.target.value })} required />
                        <InputField label="Adresse" name="address" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} required className="md:col-span-2" />
                    </>
                )}
            </div>
...
function InputField({ label, name, value, onChange, required = false, disabled = false, className = '' }: any) {
    return (
        <div className={className}>
